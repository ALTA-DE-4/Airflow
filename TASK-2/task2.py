...
dag = DAG(
        'Hasda-airflow-task2',
        description='Hello, Ini DAG Task2 Nurhasanah Darus',
...
create_table_task = PostgresOperator(
    ...
    postgres_conn_id='pg_conn_has',
    sql="""
    CREATE TABLE IF NOT EXISTS hasda_gender_name_prediction (
        input JSONB,
        details JSONB,
...
def load_predictions_to_postgres(**kwargs):
    ...
    pg_hook = PostgresHook(postgres_conn_id='pg_conn_has')
    for prediction in predictions:
        ...
        pg_hook.run("""
            INSERT INTO hasda_gender_name_prediction (input, details, result_found, first_name, probability, gender)
            VALUES (%s, %s, %s, %s, %s, %s);
